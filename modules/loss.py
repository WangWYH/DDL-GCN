...
def compute_loss(output, reports_ids, reports_masks, labels=None, vis_label=None,
                 l_img=None, l_txt=None, args={}, similarity_function='dot'):
    criterion = LanguageModelCriterion()
    r_loss = criterion(output[:, :-1], reports_ids[:, 1:], reports_masks[:, 1:]).mean()
    ...
    if args.label_loss:
        ...
        '''
        with torch.no_grad():
            vis_label = torch.sigmoid(vis_label).detach().cpu()
            y_score = vis_label.numpy()
        y = labels.cpu().detach().numpy()
        y_hat = (y_score >= 0.5).astype(int)
        precision, recall, f, _ = precision_recall_fscore_support(y, y_hat)
        # 打印评估结果
        print('\nr_loss = ', label_loss, '\nP {:.4f}, R {:.4f}, F {:.4f}'.format(precision.mean(), recall.mean(), f.mean()))
        '''

    if args.rank_loss:
        ranking_loss = RankingLoss()
        match_loss = ranking_loss(l_img, l_txt, labels, similarity_function)

    return r_loss + 0.1 * label_loss + 0.1 * match_loss
