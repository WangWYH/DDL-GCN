...
class BaseTrainer(object):
    ...
    def _print_best_to_file(self):
        ...
        for split in ['val', 'test']:
            self.best_recorder[split]['version'] = f'{self.args.version}'
    ...
    def _resume_checkpoint(self, resume_path):
        ...
        '''
        # 修改为新的学习率，继续学习
        lr_ve = self.args.lr_ve
        lr_ed = self.args.lr_ed
        weight_decay = self.args.weight_decay
        self.model.lr_ve = lr_ve
        self.model.lr_ed = lr_ed
        self.model.weight_decay = weight_decay
        for param_group in self.optimizer.param_groups:
            if param_group['params'] == list(self.model.visual_extractor.parameters()):
                param_group['lr'] = lr_ve
            else:
