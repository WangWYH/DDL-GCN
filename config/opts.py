...
def parse_opt(prefix=None):
    ...
    # Data input settings
    parser.add_argument('--image_dir', type=str, default='D:/code/database/iu_xray/images/',
                        help='the path to the directory containing the data.')
    parser.add_argument('--ann_path', type=str, default='D:/code/database/iu_xray/annotation.json',
                        help='the path to the directory containing the data.')
    parser.add_argument('--label_path', type=str, default='data/iu/r2gen_split/id_label.csv',
    ...
    parser.add_argument('--dataset_name', type=str, default='iu_xray',
                        choices=['iu_xray', 'mimic_cxr', 'covid', 'covidall'], help='the dataset to be used.')
    ...
    parser.add_argument('--pretrain_cnn_file', type=str, help='the visual extractor to be used.')
    ...
    # Optimization
    parser.add_argument('--label_loss', default=True, action='store_true')
    parser.add_argument('--rank_loss', default=True, action='store_true')
    ...
    parser.add_argument('--weight_decay', type=float, default=4e-5, help='the weight decay.')
    parser.add_argument('--expe_name', type=str, default='try', help='extra experiment name')
    ...
    # config
    parser.add_argument('--cfg', type=str, default='config/iu_efficientnet.yml',
    ...
    if args.resume == None or args.expe_name != '':
        ...
        expe_name = args.visual_extractor \
                    + ("_labelloss" if args.label_loss else "") \
                    + ("_rankloss" if args.rank_loss else "") \
                    + ("_" + args.expe_name if args.expe_name != "" else "") \
                    + (datetime.now() + timedelta(hours=15)).strftime("_%Y%m%d-%H%M%S")
